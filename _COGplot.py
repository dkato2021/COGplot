...
def get_args():
    parser = argparse.ArgumentParser(description='dkato. Sep. 2023')
...
def run_rpsblast_2(paths_to_proteins = None, 
                 path_to_cogdb = None, 
                 evalue = None, num_threads = None):
    from subprocess import Popen
    error1 = "specify the path to your Cog database with cogdb option. (default:/home/tmp/db/COG/Cog)"
    ...
    path_to_rpsRes = []
    stock=paths_to_proteins
    procs=[]
    while len(stock) != 0:
        params=stock[:num_threads]; stock=stock[num_threads:]
        for path in params:
            name = os.path.splitext(os.path.basename(path))[0]
            procs += [Popen(f"rpsblast -query {path} -db {path_to_cogdb} -out ./rps_{evalue}/{name}.txt -evalue {evalue} -outfmt 6"
                     , shell=True)]
...
def run_rpsblast2(paths_to_proteins = None, 
                 path_to_cogdb = None, 
                 evalue = None, num_threads = None):
    from subprocess import Popen
    error1 = "specify the path to your Cog database with cogdb option. (default:/home/tmp/db/COG/Cog)"
    ...
    params=stock[:num_threads]; stock=stock[num_threads:]
    procs = [Popen(f"rpsblast -query {path} -db {path_to_cogdb} -out ./rps_{evalue}/{name}.txt -evalue {evalue} -outfmt 6"
           , shell=True) for path in params]
           
    while len(stock) != 0:
        #全て実行中の場合はここでストップ
        while [None]*len(procs) == [p.poll() for p in procs]: 
            pass
        else:
            procs.pop([p.poll() for p in procs].index(0))
            path=stock[0]
            procs += [Popen(f"rpsblast -query {path} -db {path_to_cogdb} -out ./rps_{evalue}/{name}.txt -evalue {evalue} -outfmt 6"
                           , shell=True)]
            path_to_rpsRes.append(f"./rps_{evalue}/{name}.txt")
...
def main():
    ...
    for e in get_args().evalue:
        if get_args().AA is not None:
            ...
            path_to_rpsRes = run_rpsblast2(paths_to_proteins = get_args().AA, 
